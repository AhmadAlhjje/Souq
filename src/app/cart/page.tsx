...
const transformCartItems = (cartItems: any[]) => {
  return cartItems.map(item => ({
    ...
    image: item.image,
    total: (item.salePrice || item.originalPrice || item.price || 0) * item.cartQuantity,
    inStock: item.inStock !== false,
    discount: item.salePrice && item.originalPrice 
...
const CartPage: React.FC = () => {
  ...
  const deliveryFee = subtotal > 200 ? 0 : 15; // شحن مجاني للطلبات أكثر من 200 ريال
  ...
  const handleDeleteSelected = () => {
    if (selectedItems.size > 0) {
      const confirmed = window.confirm(`هل أنت متأكد من حذف ${selectedItems.size} منتج؟`);
      if (confirmed) {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-screen items-center flex justify-center bg-gradient-to-br from-gray-50 to-blue-50/30 py-8" dir="rtl">
        <div className="max-w-3xl mx-auto px-4">
          <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">أضف بعض المنتجات إلى سلتك لتبدأ التسوق</p>
            <button 
