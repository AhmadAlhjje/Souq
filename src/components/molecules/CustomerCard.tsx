// components/molecules/CustomerCard.tsx
"use client";
...
export default function CustomerCard({ src, alt, title, subtitle, percentage, color, features }: CustomerCardProps) {
  ...
  return (
    <div className={`group relative bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden hover:-translate-y-2 ${isRTL ? 'text-right' : 'text-left'}`}>
      {/* Background Effect */}
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 mb-4">
          {features.map((feature, featureIndex) => (
            <div key={featureIndex} className={`flex items-center gap-3 ${isRTL ? 'justify-start' : 'justify-start'}`} dir={isRTL ? 'rtl' : 'ltr'}>
