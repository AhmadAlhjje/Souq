  ...
  return (
    ...
    `}>
      ...
      <div className="relative overflow-hidden">
        ...
        {product.stock <= 5 && product.status !== "out_of_stock" && (
          <div className="absolute bottom-2 left-2">
            <div className={`${stockWarningClasses} px-2 py-1 rounded text-xs font-medium transition-colors duration-200`}>
      ...
      <div className="p-4">
        {/* Title and Category */}
        <div className="mb-3">
          <h3 className={`font-semibold ${titleClasses} mb-1 line-clamp-2 leading-tight transition-colors duration-200`}>
            {isRTL ? product.nameAr : product.name}
          </h3>
          <p className={`text-sm ${categoryClasses} transition-colors duration-200`}>
        ...
        <div className="flex items-center justify-between mb-3">
          <span className={`text-xl font-bold ${priceClasses} transition-colors duration-200`}>
            ${product.price.toLocaleString()}
          </span>
          <div className="flex items-center gap-1">
            <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
            <span className={`text-sm font-medium ${ratingTextClasses} transition-colors duration-200`}>
              {product.rating}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4 text-sm">
          <div className="flex items-center gap-2">
            <Package className={`w-4 h-4 ${iconClasses} transition-colors duration-200`} />
            <span className={`${stockClasses(product.stock)} transition-colors duration-200`}>
