...
const SalesChart: React.FC<SalesChartProps> = ({ ordersStats, loading }) => {
  ...
  const calculateMonthlyStats = () => {
    ...
    const lastMonthOrders = ordersStats?.allOrders?.orders?.filter((order: any) => {
      const orderDate = new Date(order.created_at);
      return orderDate >= lastMonth;
    }) || [];

    const monthlyRevenue = lastMonthOrders.reduce((sum: number, order: any) => 
    ...
    const previousMonthOrders = ordersStats?.allOrders?.orders?.filter((order: any) => {
      const orderDate = new Date(order.created_at);
      ...
      <div className="relative p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className={`text-sm mt-1 ${
              ...
              نظرة عامة على آخر 30 يوم
            </p>
          </div>
          
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl ${
            isDark ? 'bg-gray-700/50' : 'bg-[#CFF7EE]/50'
          }`}>
            <div className={`w-2 h-2 rounded-full ${
              monthlyStats.revenueGrowth >= 0 ? 'bg-green-500' : 'bg-red-500'
            }`} />
            <span className={`text-sm font-medium ${
              isDark ? 'text-gray-300' : 'text-[#5CA9B5]'
            }`}>
              {monthlyStats.revenueGrowth >= 0 ? '+' : ''}{monthlyStats.revenueGrowth.toFixed(1)}%
            </span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
                ...
                >
                  <defs>
                    <linearGradient id="revenueGradient" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <stop offset="95%" stopColor="#5CA9B5" stopOpacity={0.05}/>
                    </linearGradient>
                    <linearGradient id="ordersGradient" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#96EDD9" stopOpacity={0.3}/>
                      <stop offset="95%" stopColor="#96EDD9" stopOpacity={0.05}/>
                    </linearGradient>
                  </defs>
            ...
            {/* إحصائيات محسنة */}
            <div className={`
              grid grid-cols-3 gap-4 p-4 rounded-xl border
              ${isDark 
            ...
            `}>
              ...
              >
                ...
                <div className={`text-lg font-bold ${
                  ...
                  {monthlyStats.monthlyRevenue.toLocaleString('ar-SA')} 
                  <span className="text-sm font-normal ml-1">ل.س</span>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className={`text-lg font-bold ${
                  ...
                  <span className="text-sm font-normal ml-1">طلب</span>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className={`text-lg font-bold ${
                  ...
                  {monthlyStats.avgOrderValue.toFixed(0)}
                  <span className="text-sm font-normal ml-1">ل.س</span>
                </div>
              </motion.div>
            </div>
          ...
          >
            ...
            >
              ...
              >
                <svg 
                  className={`w-12 h-12 ${isDark ? 'text-gray-300' : 'text-[#5CA9B5]'}`} 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
