...
const ProductsSection: React.FC<ProductsSectionProps> = ({
  ...
  const handleSearchResults = useCallback((results: ProductSearchResponse | null) => {
...
const convertApiProductToLocal = useCallback((apiProduct: ApiProduct): Product => {
  ...
  if (raw) {
    try {
      ...
      if (first && first !== "null" && first !== "") {
        ...
        else if (first.startsWith("uploads/")) imageUrl = `${baseUrl}/${first}`;
  ...
  const availableProducts = displayedProducts.filter((p) => p.inStock && p.stock > 0);
  const discountedProducts = displayedProducts.filter((p) => p.hasDiscount === true);
  const newProducts = displayedProducts.filter((p) => p.isNew);
  const outOfStockProducts = displayedProducts.filter((p) => p.status === 'out_of_stock');
  const lowStockProducts = displayedProducts.filter((p) => p.status === 'low_stock');
  ...
  return (
    <div className="lg:col-span-3">
      <div className="p-6 rounded-2xl shadow-lg" style={{ backgroundColor: '#CFF7EE' }}>
        ...
        <div className="flex items-center justify-center mb-8">
          <div className="flex flex-col items-center gap-3">
            <h2 className="text-3xl font-bold text-center text-gray-900 flex items-center gap-3">
              ...
              <span>{storeId && storeName ? `منتجات ${storeName}` : 'جميع المنتجات'}</span>
            </h2>
