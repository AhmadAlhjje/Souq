...
const ProductsTable: React.FC<ProductsTableProps> = ({
  ...
  const rowBorderClasses = isDark ? "border-gray-700" : "border-gray-100";

  const rowHoverClasses = isDark ? "hover:bg-gray-700" : "hover:bg-gray-50";

  const productTitleClasses = isDark ? "text-gray-200" : "text-[#004D5A]";

  const categoryTextClasses = isDark ? "text-gray-400" : "text-gray-500";

  const priceClasses = isDark ? "text-gray-200" : "text-[#004D5A]";

  const ratingTextClasses = isDark ? "text-gray-300" : "text-gray-900";

  const salesTextClasses = isDark ? "text-gray-300" : "text-gray-600";
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            >
              <tr>
                <th
                  className={`py-4 px-4 text-left ${headerTextClasses} font-semibold transition-colors duration-200`}
                >
                  {t("headers.product")}
                </th>
                <th
            ...
            {products.map((product: Product) => (
              ...
              >
                <td className="py-4 px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p
                        className={`text-sm ${categoryTextClasses} transition-colors duration-200`}
                      >
                ...
                <td className="py-4 px-4">
                  ...
                  >
                    {getStatusText(product.status ?? "unknown")}
                  </span>
                </td>
                <td className="py-4 px-4">
                  <div className="flex items-center gap-1">
                    ...
                    <span
                      className={`text-sm ${ratingTextClasses} transition-colors duration-200`}
                    >
                      {product.rating}
                    </span>
                  </div>
                ...
                <td className="py-4 px-4">
                  <span
                    className={`${salesTextClasses} transition-colors duration-200`}
                  >
                    {product.sales}
                  </span>
                </td>
