...
const cartReducer = (state: CartState, action: CartAction): CartState => {
  ...
  switch (action.type) {
    ...
    case "ADD_TO_CART": {
      ...
      if (!product.inStock) {
        console.warn(
          `Product "${product.name}" is out of stock and cannot be added to cart.`
        );
        return state;
      }
      ...
      if (quantity > (product.stock ?? 0)) {
        console.warn(
          `Requested quantity (${quantity}) exceeds available stock (${product.stock}) for product "${product.name}".`
        );
        return state;
      }
      ...
      if (existingItem) {
        ...
        if (totalAfterAdd > (product.stock ?? 0)) {
          console.warn(
            `Total quantity after add (${totalAfterAdd}) exceeds available stock (${product.stock}) for product "${product.name}".`
          );
          return state;
        }
      }
    ...
    case "UPDATE_QUANTITY": {
      ...
      if (existingItem && quantity > (existingItem.stock ?? 0)) {
        console.warn(
          `Quantity ${quantity} exceeds available stock ${existingItem.stock} for product ${existingItem.name}`
        );
        return state;
      }
